...
def load_data_from_directory(directory, max_frame=None, config=None):
    """
    ...
    Args:
        ...
        config: Configuration dictionary containing selected_features
    """
    all_features = []
    lifespans = []
    lengths = []
    ...
    for filename in tqdm(files, desc=f"Loading {os.path.basename(directory)}", leave=False):
        ...
        # Get feature indices based on selected features
        feature_names = data['feature_names']
        if config and 'selected_features' in config['training']:
            selected_indices = [i for i, name in enumerate(feature_names) 
                              if name in config['training']['selected_features']]
            if not selected_indices:
                raise ValueError(f"None of the selected features {config['training']['selected_features']} "
                               f"found in available features {feature_names}")
        else:
            selected_indices = slice(None)  # Use all features if no selection
        
        if max_frame is not None:
            # Load the original CSV to get frame numbers
            original_dir = directory.replace('Lifespan_features', 'Lifespan_cleaned')
            ...
            for i, cluster_id in enumerate(range(len(features))):
                if cluster_id in valid_clusters:
                    if isinstance(selected_indices, list):
                        cluster_features.append([features[i][j] for j in selected_indices])
                    else:
            ...
            features = data['features']
            if isinstance(selected_indices, list):
                features = np.array([[feat[j] for j in selected_indices] for feat in features])
            
            all_features.append(features)
            lifespans.append(float(data['num_frames']))
            lengths.append(int(len(features)))
            file_paths.append(filepath)
            cluster_counts.append((os.path.basename(filepath), total_clusters, total_clusters))
            groups.append(os.path.basename(directory))  # Store the group name
    
    if not all_features:
        raise ValueError(f"No valid features found in directory {directory}")
    
    # Print feature selection info
    if config and 'selected_features' in config['training']:
        print(f"\nUsing {len(config['training']['selected_features'])} selected features:")
        for feat in config['training']['selected_features']:
            print(f"- {feat}")
    else:
...
def evaluate_model(model, val_loader, criterion, device, config):
    ...
    lifespan_mean = config['training']['lifespan_mean']
    lifespan_std = config['training']['lifespan_std']
    ...
    with torch.no_grad():
        for batch_features, batch_lifespans, batch_lengths in val_loader:
            ...
            outputs_np = outputs.cpu().numpy() * lifespan_std + lifespan_mean
            batch_lifespans_np = batch_lifespans.cpu().numpy() * lifespan_std + lifespan_mean
...
def train_fold(train_loader, val_loader, input_size, config, device, fold_num=None):
    ...
    for epoch in tqdm(range(config['training']['num_epochs']), desc=f"{fold_desc} epochs"):
        ...
        if early_stopping(val_loss):
            tqdm.write(f'{fold_desc} - Early stopping triggered at epoch {epoch+1}')
            break
...
def k_fold_cross_validation(features, lifespans, lengths, files, groups, feature_scaler, config, input_size):
    ...
    # Calculate mean and std of lifespans for normalization
    lifespans_array = np.array(lifespans)
    lifespan_mean = np.mean(lifespans_array)
    lifespan_std = np.std(lifespans_array)
    
    # Store normalization values in config for later use
    config['training']['lifespan_mean'] = float(lifespan_mean)
    config['training']['lifespan_std'] = float(lifespan_std)
    ...
    print("\nTarget variable (lifespan) normalization parameters:")
    ...
    for fold in range(k):
        ...
        # Scale lifespans using mean-std normalization
        scaled_train_lifespans = (np.array(train_lifespans) - lifespan_mean) / lifespan_std
        scaled_val_lifespans = (np.array(val_lifespans) - lifespan_mean) / lifespan_std
...
def main():
    # Add argument parser
    parser = argparse.ArgumentParser(description='Train lifespan prediction model.')
    parser.add_argument('--input', type=str, required=True, help='Input directory containing feature data')
    parser.add_argument('--config', type=str, default='train_model_config.json', help='Path to configuration file')
    parser.add_argument('--output', type=str, help='Directory for saving model outputs (optional)', default='model_outputs')
    ...
    CONFIG['data']['base_dir'] = args.input
    ...
    for subdir in tqdm(subdirs, desc="Processing directories"):
        ...
        features, lifespans, lengths, file_paths, cluster_counts, groups = load_data_from_directory(
            dir_path, max_frame=max_frame, config=CONFIG
        )
    ...
    print(f"\nInput size (number of features): {input_size}")
